...
public class IncidentDao {
	...
	public List<IncidentDo> sortrisklevel() {
		try {
			Session session = this.factory.getCurrentSession();
			List<IncidentDo> incidentList = session
					.createQuery("from IncidentDo where riskLevel >= 5 order by INCIDENT_ID asc").list();
			return incidentList;
		} catch (Exception e) {
			System.err.println("ERROR:---" + e);
		}
	...
	public List<IncidentDo> getNewIncidents() {
		try {
			Session session = this.factory.getCurrentSession();
			List<IncidentDo> incidentList = session
					.createQuery("from IncidentDo where flag = 0 OR riskLevel < 5 order by INCIDENT_ID asc").list();
			return incidentList;
		} catch (Exception e) {
			System.err.println("ERROR:---" + e);
		}
	...
	public String updateRisk(IncidentDo incident) {
		if (incident != null) {
			...
			Query query = session.createQuery(
					"update IncidentDo set RISK_LEVEL=:RISK_LEVEL,FLAG=:FLAG where INCIDENT_ID=:INCIDENT_ID");
	...
	public String rejectStatus(IncidentDo incident) {
		if (incident != null) {
			Session session = this.factory.getCurrentSession();
			Query query = session.createQuery(
					"update IncidentDo set STATUS=:STATUS,ACTION_TEAM_STATUS=:ACTION_TEAM_STATUS where INCIDENT_ID=:INCIDENT_ID");
	...
	public String saveComments(IncidentDo incident) {
		if (incident != null) {
			Session session = this.factory.getCurrentSession();
			Query query = session
					.createQuery("update IncidentDo set COMMENTS=:COMMENTS where INCIDENT_ID=:INCIDENT_ID");
	...
	public String updateAll(IncidentDo incident) {
		if (incident != null) {
			Session session = this.factory.getCurrentSession();
			Query query = session.createQuery(
					"update IncidentDo set LOCATION=:LOCATION, DESCRIPTION=:DESCRIPTION,INCIDENT_TYPE=:INCIDENT_TYPE,INCIDENT_DATE=:INCIDENT_DATE,SECTOR=:SECTOR,RISK_LEVEL=:RISK_LEVEL,EMPLOYEE_NAME=:EMPLOYEE_NAME,INJURY=:INJURY,ACTION_NEEDED=:ACTION_NEEDED where INCIDENT_ID=:INCIDENT_ID");
	...
	public String updateAction(IncidentDo incident) {
		if (incident != null) {
			Session session = this.factory.getCurrentSession();
			Query query = session.createQuery(
					"update IncidentDo set ACTION_TEAM_STATUS=:ACTION_TEAM_STATUS where INCIDENT_ID=:INCIDENT_ID");
	...
	public List<IncidentDo> getDoneincidents() {
		try {
			Session session = this.factory.getCurrentSession();
			List<IncidentDo> incidentList = session
					.createQuery("from IncidentDo where actionTeamStatus='Completed' order by INCIDENT_ID asc").list();
			return incidentList;
		} catch (Exception e) {
			System.err.println("ERROR:---" + e);
		}
	...
	public List<IncidentDo> getApprovedincidents() {
		try {
			Session session = this.factory.getCurrentSession();
			List<IncidentDo> incidentList = session
					.createQuery("from IncidentDo where status='Approved' order by INCIDENT_ID asc").list();
			return incidentList;
		} catch (Exception e) {
			System.err.println("ERROR:---" + e);
		}
	...
	public List<IncidentDo> getGraphdata() {
		try {
			Session session = this.factory.getCurrentSession();
			List<IncidentDo> incidentList = session
					.createQuery("SELECT incidentType, MAX(riskLevel) FROM IncidentDo GROUP BY incidentType").list();
			return incidentList;
		} catch (Exception e) {
			System.err.println("ERROR:---" + e);
		}
	...
	// getting incidents with processing or no action
	@SuppressWarnings("unchecked")
	public List<IncidentDo> getProcessingIncidents() {
		try {
			Session session = this.factory.getCurrentSession();
			List<IncidentDo> incidentList = session
					.createQuery(
							"from IncidentDo where (actionTeamStatus='No Action' or actionTeamStatus='Processing..') and status='Approved' order by INCIDENT_ID asc")
					.list();
			return incidentList;
		} catch (Exception e) {
			System.err.println("ERROR:---" + e);
	...
	// get audit team dashboard display
	@SuppressWarnings("unchecked")
	public List<IncidentDo> auditdisplay() {
		try {
			Session session = this.factory.getCurrentSession();
			List<IncidentDo> incidentList = session
					.createQuery(
							"from IncidentDo where riskLevel >= 5 and status= 'In Progress' order by INCIDENT_ID asc")
					.list();
			return incidentList;
		} catch (Exception e) {
			System.err.println("ERROR:---" + e);
	...
	// public String savePhoto(MultipartFile multipartImage, int id) throws
	// IOException{
	//
	// Session session = this.factory.getCurrentSession();
	// Query query = session.createQuery("update IncidentDo set PHOTO=:PHOTO
